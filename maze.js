...
Block.prototype.neighbors = function() {
  ...
  this._neighbors = [];
  if (this.x > 0) {
    this._neighbors.push(this.neighbor(-1, 0)); //left direction relatively
  }

  if (this.x < this.maze.hBlocks - 1) {
    this._neighbors.push(this.neighbor(1, 0)); //right direction relatively
  }

  if (this.y > 0) {
    this._neighbors.push(this.neighbor(0, -1)); //downwards relatively
  }

  if (this.y < this.maze.vBlocks - 1) {
    this._neighbors.push(this.neighbor(0, 1)); //above relatively
  }
...
Block.prototype.erase = function() {
  this.ctx.fillStyle = "#FFFFFF"; //white fillstyle
...
Block.prototype.drawWall = function(x1, y1, x2, y2) {
  var ctx = this.ctx;
  ctx.strokeStyle = "#000000"; //black strokestyle
  ctx.beginPath();
  ctx.moveTo(x1, y1);
  ctx.lineTo(x2, y2);
  ctx.closePath();
...
var MazeGenerator = function(width, height, hBlocks, vBlocks, interval, steps, id) {
  ...
  this.ctx = this.canvas.getContext('2d');//returns a drawing context on the canvas
...
MazeGenerator.prototype.step = function() {
  ...
  if (!currentBlock) { //if there is no current block then go back to the old block
    oldBlock.draw();
    this.finished = true;
    return;
  }
...
function initMaze() {
  if (window.maze) {
    clearTimeout(window.maze.timeout); //if maze has already been created then clear it's timeout, otherwise create the maze 
  }
  ...
  var steps = parseInt(document.getElementById('steps').value, 10); //all these are user inputted
    window.maze = new MazeGenerator(canvasSize, canvasSize, blocks, blocks, interval, steps, 'maze'); // call to function MazeGenerator
    // the function forms a square with the above dimensions
    window.maze.drawLoop();// call to function drawLoop()
}
